...
interface Props { open:boolean; title:string; children:ReactNode; onClose:()=>void; onConfirm:()=>void; confirmLabel?:string; danger?:boolean }
export default function Modal({ open,title,children,onClose,onConfirm,confirmLabel='Confirmar',danger=false }:Props) {
  ...
  return (
    <div onClick={onClose} style={{position:'fixed',inset:0,zIndex:9000,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',padding:16}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'#0d1218',border:'1px solid #243447',borderRadius:16,width:'100%',maxWidth:560,maxHeight:'90vh',overflow:'auto',boxShadow:'0 24px 80px rgba(0,0,0,0.8)',animation:'scaleIn 0.25s ease'}}>
        ...
        <div style={{padding:'14px 24px',borderTop:'1px solid #1e2d3d',display:'flex',justifyContent:'flex-end',gap:10}}>
          ...
          <button onClick={onConfirm} style={{padding:'9px 18px',border:danger?'1px solid rgba(255,71,87,0.3)':'none',borderRadius:8,background:danger?'rgba(255,71,87,0.15)':'linear-gradient(135deg,#00e676,#00c853)',color:danger?'#ff4757':'#080c10',fontSize:13,fontWeight:700,cursor:'pointer'} as React.CSSProperties}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  )
