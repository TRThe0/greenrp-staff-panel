  ...
  return (
    <Layout page="avisos">
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div><div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:700,color:'#f0f4f8'}}>Avisos Internos</div><div style={{fontSize:13,color:'#6b7f93',marginTop:2}}>Comunicados para a equipe</div></div>
        {session?.perm==='admin'&&<button onClick={()=>setModal(true)} style={{padding:'9px 18px',border:'none',borderRadius:8,background:'linear-gradient(135deg,#00e676,#00c853)',color:'#080c10',fontSize:13,fontWeight:700,cursor:'pointer'}}>+ Novo Aviso</button>}
      </div>
      ...
      {avisos.map((a,i)=>(
        <div key={i} style={{background:'#111820',border:'1px solid #1e2d3d',borderLeft:`3px solid ${bc(a.tipo)}`,borderRadius:12,padding:'16px 20px',marginBottom:12,display:'flex',gap:12,alignItems:'flex-start'}}>
          <div style={{flex:1}}>
            ...
            <div style={{fontSize:11,color:'#6b7f93'}}>Por <strong style={{color:'#a8b8c8'}}>{a.autor}</strong> · {timeAgo(a.data)}</div>
          </div>
          {session?.perm==='admin'&&<button onClick={()=>remove(a.id)} style={{width:30,height:30,borderRadius:8,background:'rgba(255,71,87,0.1)',border:'1px solid rgba(255,71,87,0.2)',color:'#ff4757',cursor:'pointer',flexShrink:0,fontSize:13}}>🗑</button>}
        </div>
      ))}
      <Modal open={modal} title="Novo Aviso" onClose={()=>setModal(false)} onConfirm={save} confirmLabel="Publicar">
        <div style={{display:'grid',gap:14}}>
          <div><label style={{fontSize:12,color:'#6b7f93',display:'block',marginBottom:6}}>Tipo</label><select style={inp} value={form.tipo} onChange={e=>setForm(f=>({...f,tipo:e.target.value}))}><option value="info">Informativo</option><option value="warn">Atenção</option><option value="danger">Urgente</option></select></div>
          <div><label style={{fontSize:12,color:'#6b7f93',display:'block',marginBottom:6}}>Mensagem</label><textarea style={{...inp,resize:'vertical'}} rows={4} placeholder="Escreva o aviso..." value={form.msg} onChange={e=>setForm(f=>({...f,msg:e.target.value}))}/></div>
        </div>
      </Modal>
    </Layout>
  )
