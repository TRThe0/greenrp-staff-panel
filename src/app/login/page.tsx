...
export default function LoginPage() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if (remember) localStorage.setItem('grp_session',session); else sessionStorage.setItem('grp_session',session)
      localStorage.setItem('grp_user',JSON.stringify(data.staff))
    ...
    <div style={{minHeight:'100vh',background:'#080c10',display:'flex',alignItems:'center',justifyContent:'center',padding:16,backgroundImage:'radial-gradient(ellipse 80% 60% at 50% 0%,rgba(0,230,118,0.07) 0%,transparent 70%)'}}>
      <div style={{width:'100%',maxWidth:420}} className="fade-up">
        <div style={{textAlign:'center',marginBottom:36}}>
          ...
          <div style={{fontSize:14,color:'#6b7f93',marginTop:4}}>Staff Management Panel</div>
        </div>
        <div style={{background:'#111820',border:'1px solid #1e2d3d',borderRadius:16,padding:32,boxShadow:'0 20px 60px rgba(0,0,0,0.5)'}}>
          ...
          <label style={{display:'block',fontSize:13,fontWeight:500,color:'#a8b8c8',marginBottom:8}}>Nome de Usuário</label>
          <input style={inp} type="text" placeholder="Seu usuário" value={username} onChange={e=>setUsername(e.target.value)} onKeyDown={e=>e.key==='Enter'&&handleLogin()}/>
