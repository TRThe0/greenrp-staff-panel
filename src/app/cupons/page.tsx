...
export default function CuponsPage() {
  ...
  const sorted = [...staffs].sort((a,b)=>b.valorGerado-a.valorGerado)
  const maxVal = sorted[0]?.valorGerado || 1
  const isAdmin = session?.perm === 'admin'

  const totalUsos = staffs.reduce((a,s)=>a+s.usos,0)
  const totalGerado = staffs.reduce((a,s)=>a+s.valorGerado,0)
  const totalComissao = staffs.reduce((a,s)=>a+(s.valorGerado*(s.pct/100)),0)
    ...
    <Layout page="cupons">
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div>
          ...
          <div style={{fontSize:13,color:'#6b7f93',marginTop:2}}>Ranking e histórico de uso</div>
        </div>
        <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
          <button onClick={copyWebhook} style={{padding:'9px 16px',border:'1px solid rgba(79,172,254,0.3)',borderRadius:8,background:'rgba(79,172,254,0.1)',color:'#4facfe',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:6}}>
            {copied ? '✅ Copiado!' : '🔗 Copiar URL da API'}
          </button>
          {isAdmin && (
            <button onClick={resetar} disabled={resetting} style={{padding:'9px 16px',border:'1px solid rgba(255,71,87,0.3)',borderRadius:8,background:'rgba(255,71,87,0.1)',color:'#ff4757',fontSize:13,fontWeight:600,cursor:'pointer',display:'flex',alignItems:'center',gap:6,opacity:resetting?0.6:1}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:14,marginBottom:24}}>
        {[
          ...
          {label:'Valor Total Gerado',val:`R$ ${totalGerado.toLocaleString('pt-BR',{minimumFractionDigits:2})}`,color:'#00e676',icon:'💰'},
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(260px,1fr))',gap:16,marginBottom:28}}>
        {sorted.map(s=>{
          const comissao = s.valorGerado * (s.pct / 100)
          const pct = Math.round((s.valorGerado / maxVal) * 100)
          return (
            <div key={s.id} style={{background:'#111820',border:'1px solid #1e2d3d',borderRadius:12,padding:20,transition:'all 0.2s'}}
              ...
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,marginBottom:14}}>
                ...
                <div style={{background:'#161f2a',borderRadius:8,padding:'10px 10px'}}>
                  <div style={{fontSize:13,fontWeight:700,color:'#4facfe',fontFamily:'Syne,sans-serif'}}>R$ {(s.valorGerado||0).toLocaleString('pt-BR',{minimumFractionDigits:2})}</div>
      ...
      {/* WEBHOOK INFO */}
      <div style={{background:'rgba(79,172,254,0.06)',border:'1px solid rgba(79,172,254,0.2)',borderRadius:12,padding:'16px 20px',marginBottom:24}}>
        <div style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:600,color:'#4facfe',marginBottom:8}}>🔗 Integração com sua Loja</div>
        <div style={{fontSize:13,color:'#a8b8c8',marginBottom:12,lineHeight:1.7}}>
  ...
  "cupom": "BRUNO2024",
  "usadoPor": "NomeDoCliente",
  "valorCompra": 150.00
}`}</pre>
        <div style={{fontSize:12,color:'#6b7f93',marginTop:8}}>O sistema calcula automaticamente a comissão do staff com base na porcentagem do cupom.</div>
      </div>
      ...
      <div style={{background:'#111820',border:'1px solid #1e2d3d',borderRadius:12,overflow:'hidden'}}>
        <div style={{overflowX:'auto'}}>
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>
              {!history.length && <tr><td colSpan={6} style={{textAlign:'center',padding:40,color:'#6b7f93',fontSize:13}}>Nenhum uso registrado</td></tr>}
              {[...history].reverse().map((h,i)=>{
                const staff = staffs.find(s=>s.cupom===h.cupom)
                const comissao = staff ? h.valor*(staff.pct/100) : 0
                return (
                  <tr key={i} style={{borderBottom:'1px solid #1e2d3d'}} onMouseEnter={e=>(e.currentTarget.style.background='#161f2a')} onMouseLeave={e=>(e.currentTarget.style.background='transparent')}>
                    <td style={{padding:'13px 18px'}}><span style={{fontFamily:'monospace',fontSize:13,fontWeight:700,color:'#ffa502'}}>{h.cupom}</span></td>
                    <td style={{padding:'13px 18px',fontSize:13,fontWeight:600,color:'#f0f4f8'}}>{h.staff}</td>
                    <td style={{padding:'13px 18px',fontSize:13,color:'#a8b8c8'}}>{h.usadoPor}</td>
                    ...
                    <td style={{padding:'13px 18px',fontSize:13,color:'#6b7f93'}}>{timeAgo(h.data)}</td>
                  </tr>
                )
              })}
            </tbody>
