...
export default function DashboardPage() {
  ...
  if (loading) return <Layout page="dashboard"><div style={{textAlign:'center',padding:60,color:'#6b7f93'}}>Carregando...</div></Layout>

  return (
    <Layout page="dashboard">
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:16,marginBottom:24}}>
        {[{label:'Total de Staffs',val:staffs.length,color:'#00e676',icon:'👥'},{label:'Staffs Online',val:staffs.filter(s=>s.online).length,color:'#4facfe',icon:'🟢'},{label:'Cupons Usados',val:cupons.length,color:'#ffa502',icon:'🎫'},{label:'Promovidos',val:promovidos.length,color:'#b388ff',icon:'⬆️'}].map((s,i)=>(
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <div style={card}>
          ...
          {sorted.slice(0,5).map((s,i)=>(
            <div key={s.id} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:i<4?'1px solid #1e2d3d':'none'}}>
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600,color:'#f0f4f8'}}>{s.nome} <span style={{fontSize:11,color:'#6b7f93',fontWeight:400}}>{s.cargo}</span></div>
                <div style={{fontSize:12,color:'#6b7f93'}}>{s.usos} usos · R$ {(s.valorGerado||0).toLocaleString('pt-BR')}</div>
                <div style={{height:3,background:'#1a2535',borderRadius:99,marginTop:5,overflow:'hidden'}}><div style={{height:'100%',width:Math.round((s.usos/maxUsos)*100)+'%',background:'linear-gradient(90deg,#00e676,#69f0ae)',borderRadius:99}}></div></div>
              </div>
            </div>
          ))}
        </div>
        <div style={card}>
          ...
          {[...promovidos].slice(-4).reverse().map((p,i)=>(
            <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 0',borderBottom:i<3?'1px solid #1e2d3d':'none'}}>
              ...
              <div><div style={{fontSize:13,fontWeight:600,color:'#f0f4f8'}}>{p.nome}</div><div style={{fontSize:12,color:'#6b7f93'}}>{p.de} → <span style={{color:'#00e676'}}>{p.para}</span></div></div>
            </div>
          ))}
        </div>
        <div style={card}>
          ...
          {avisos.slice(0,3).map((a,i)=>(
            <div key={i} style={{background:'#161f2a',border:'1px solid #1e2d3d',borderLeft:`3px solid ${a.tipo==='danger'?'#ff4757':a.tipo==='warn'?'#ffa502':'#00e676'}`,borderRadius:8,padding:'12px 14px',marginBottom:8}}>
              ...
              <div style={{fontSize:11,color:'#6b7f93',marginTop:4}}>Por {a.autor} · {timeAgo(a.data)}</div>
            </div>
          ))}
        </div>
        <div style={card}>
          <div style={{fontFamily:'Syne,sans-serif',fontSize:14,fontWeight:600,marginBottom:16,color:'#f0f4f8'}}>🕐 Últimos Acessos</div>
          {[...staffs].filter(s=>s.ultimoAcesso).sort((a,b)=>new Date(b.ultimoAcesso).getTime()-new Date(a.ultimoAcesso).getTime()).slice(0,6).map((s,i,arr)=>(
            <div key={s.id} style={{display:'flex',alignItems:'center',gap:10,padding:'8px 0',borderBottom:i<arr.length-1?'1px solid #1e2d3d':'none'}}>
              ...
              <div style={{flex:1}}><div style={{fontSize:13,fontWeight:500,color:'#f0f4f8'}}>{s.nome}</div><div style={{fontSize:11,color:'#6b7f93'}}>{timeAgo(s.ultimoAcesso)}</div></div>
