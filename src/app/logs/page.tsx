  ...
  return (
    <Layout page="logs">
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div><div style={{fontFamily:'Syne,sans-serif',fontSize:18,fontWeight:700,color:'#f0f4f8'}}>Logs do Sistema</div><div style={{fontSize:13,color:'#6b7f93',marginTop:2}}>Histórico completo de alterações</div></div>
      ...
      <div style={{background:'#111820',border:'1px solid #1e2d3d',borderRadius:12,overflow:'hidden'}}>
        <div style={{display:'flex',gap:10,padding:'14px 18px',borderBottom:'1px solid #1e2d3d',flexWrap:'wrap'}}>
          ...
          <select value={type} onChange={e=>setType(e.target.value)} style={{background:'#161f2a',border:'1px solid #1e2d3d',borderRadius:8,padding:'8px 12px',color:'#a8b8c8',fontSize:13,outline:'none',cursor:'pointer',fontFamily:'DM Sans,sans-serif'}}>
            ...
            <option value="login">Login</option><option value="add">Adição</option>
            <option value="edit">Edição</option><option value="remove">Remoção</option>
            <option value="promo">Promoção</option><option value="aviso">Aviso</option>
          </select>
        </div>
        ...
        {filtered.map((l,i)=>(
          <div key={i} style={{display:'flex',alignItems:'flex-start',gap:14,padding:'14px 20px',borderBottom:'1px solid #1e2d3d'}}>
            ...
            <div><div style={{fontSize:13,color:'#a8b8c8'}} dangerouslySetInnerHTML={{__html:l.msg}}/><div style={{fontSize:11,color:'#6b7f93',marginTop:3}}>{formatDateTime(l.time)}</div></div>
          </div>
        ))}
      </div>
    </Layout>
